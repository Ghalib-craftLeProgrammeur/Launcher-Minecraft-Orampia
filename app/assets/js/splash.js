...
class Splash {
  constructor(){
    ...
    this.java = new Java();
    if(localStorage.getItem(".Orampia") == null) localStorage.setItem(".Orampia",  join(process.platform == 'win32' ? process.env.APPDATA : process.platform == "darwin" ? join(process.env.HOME, "Library", "Application Support") : process.env.HOME, process.platform == "darwin" ? "Orampia" : ".Orampia").replace(/\\/g, "/"));
    this.javaDefaultPath = localStorage.setItem("java", join(localStorage.getItem(".Orampia"), "runtime", "java", "bin", process.platform == "win32" ? "javaw.exe" : "java"));
  ...
  async startAnimation(){
    let splashes = [
      ...
      { "message": "Démarrage de l'IA de Orampia...", "author": "Wistaro" },
